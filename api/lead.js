...
const TEMPLATE_ENV_NAMES = {
  volunteer: 'TEMPLATE_VOLUNTEER',
  donor: 'TEMPLATE_DONOR',
  sponsor: 'TEMPLATE_SPONSOR',
  collaboration: 'TEMPLATE_COLLAB',
  default: 'TEMPLATE_COLLAB',
};
...
function resolveTemplate(intent) {
  ...
  const envName = TEMPLATE_ENV_NAMES[intent] || TEMPLATE_ENV_NAMES.default;
  if (!templateId) {
    const error = new Error(`Missing ${envName} env var`);
    error.envName = envName;
    throw error;
  }
  const from = FROM_MAP[intent] || FROM_MAP.default;
  return { templateId, from, envName };
}
...
async function sendAutoReply({ templateId, from }, to) {
  return resend.emails.send({
    from,
    to,
...
module.exports = async (req, res) => {
  ...
  let template;
  try {
    template = resolveTemplate(intent);
  } catch (error) {
    console.error('Lead template error:', error?.message || error, error?.response);
    return res
      .status(500)
  ...
  try {
    ...
    console.error('Lead admin email error:', error?.message || error, error?.response);
    return res.status(500).json({ ok: false, error: 'Failed to send admin notification.' });
  }

  try {
    await sendAutoReply(template, submitterEmail);
    return res.status(200).json({ ok: true });
  } catch (error) {
    console.error('Lead auto-reply error:', error?.message || error, error?.response);
    return res
      .status(502)

...
function resolveTemplate() {
  ...
  if (!templateId) {
    const error = new Error('Missing TEMPLATE_NEWSLETTER env var');
    error.envName = 'TEMPLATE_NEWSLETTER';
    throw error;
  }
  return templateId;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.error('Newsletter template error:', error?.message || error, error?.response);
    return res
      .status(500)
  ...
  try {
    ...
    console.error('Newsletter admin email error:', error?.message || error, error?.response);
  ...
  try {
    ...
    console.error('Newsletter auto-reply error:', error?.message || error, error?.response);
    return res
      .status(502)
